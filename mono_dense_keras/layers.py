...
# %% auto 0
__all__ = ['get_saturated_activation_function', 'get_activation_functions', 'apply_activations',
           'check_monotonicity_indicator_values', 'apply_monotonicity_indicator_to_kernel', 'MonotonicDense']
...
def get_saturated_activation_function(
    convex_activation: Callable[[tf.Tensor], tf.Tensor],
    concave_activation: Callable[[tf.Tensor], tf.Tensor],
) -> Callable[[tf.Tensor], tf.Tensor]:
    """Constructs saturated monotonic function form a pair of monotonic convex and concave activation functions.

    We assume that the concave activation function is a point reflextion of the convex activation function.

    Params:
        x: input tensor
        convex_activation: input convex activation function
        concave_activation: input concave activation function
...
def get_activation_functions(
    ...
    """Constructs convex, concave and saturated functions from a convex function name or callable

    Params:
        activation: the name or callable of monotonic convex activation function

    Returns:
        Tuple of convex, concave and saturated functions
    """
    if activation:
        ...
        saturated_activation = get_saturated_activation_function(
...
def apply_activations(
    ...
    *,
    ...
    """Applies activations to a tensor

    Activations are constructed from the name or the callable of a convex activation functions. There are three options:

    * if **is_convex** is set, we use only convex activation function,

    * if **is_concave** is set, we use only concave activation function, and

    * otherwise, we use a mixture of convex, concave and saturated activation functions according to their
        relative weights.

    Params:
        x: input tensor with the last dimension equal to **units**
        units: number of units of the **MonotonicDense** layer
        activation: the name or callable of the convex activation function
        is_convex: if set, use convex activation function only
        is_concave: if set, use concave activation function only
        activation_weights: relative weights of convex, concave and saturated activation function to be used
            is neither **is_convex** nor **is_concave** is set

    Returns:
        A tensor of the same shape of **x** where each component is the result of applying on the activation
        functions to it.
    """
    (
        convex_activation,
